...
<%int id_personne=op.GetIdPersonne(id_bien); %>
<%int id_locataire=(int)session.getAttribute("id_personne"); %>
...
<%if(session.getAttribute("typebien").equals("maison")){ %>
     C'est un bien de type <%=session.getAttribute("typebien")%>,construit  :<%=op.GetContenueCol("ajoute4", id_bien)%><br>
...
Si le locataire  de ce bien retarde le paiement il doit rembourcer:<%=op.GetContenueCol("ajoute2", id_bien)%><br>
Chaque mois le loyer sera payable(le cas du mode paiement est mensuel) :<%=op.GetContenueCol("ajoute3", id_bien)%><br>
...
Si le locataire  de ce bien  retarde le paiement il doit rembourcer:<%=op.GetContenueCol("ajoute2", id_bien)%><br>
Chaque mois le loyer sera payable(le cas du mode paiement est mensuel) :<%=op.GetContenueCol("ajoute3", id_bien)%><br>
  <img  align="top" width="20" height="20"  src="facade.png">Nombre chambre est :<%=op.GetAppartement(id_bien).getNbr_chambre() %>chambre/s <br>
  <img align="top" width="20" height="20"  src="etage.png"> Etage  :<%=op.GetAppartement(id_bien).getEtage() %>etage/s<br>
 <%} %>
...
Si le locataire  de ce bien  retarde le paiement il doit rembourcer:<%=op.GetContenueCol("ajoute2", id_bien)%><br>
Chaque mois le loyer sera payable(le cas du mode paiement est mensuel) :<%=op.GetContenueCol("ajoute3", id_bien)%><br>
     <img align="top" width="20" height="20"  src="box.png"> Capacite de Stockage est :<%=op.GetDepot(id_bien).getCapacite_stockage() %><sub> boites</sub><br>
     <img align="top" width="20" height="20"  src="depot.png"> Type   :<%=op.GetDepot(id_bien).getType() %><br>
 <%} %>
...
Si le locataire  de ce bien  retarde le paiement il doit rembourcer:<%=op.GetContenueCol("ajoute2", id_bien)%><br>
Chaque mois le loyer sera payable(le cas du mode paiement est mensuel) :<%=op.GetContenueCol("ajoute3", id_bien)%><br>
   <img align="top" width="20" height="20"  src="garage.png">  Capacite de Vehicule est :<%=op.GetGarage(id_bien).getCapacite_vehicule() %> <sub>vehicule/s</sub><br>
 <%} %>
...
Si le locataire  de ce bien retarde le paiement il doit rembourcer:<%=op.GetContenueCol("ajoute2", id_bien)%><br>
Chaque mois le loyer sera payable(le cas du mode paiement est mensuel) :<%=op.GetContenueCol("ajoute3", id_bien)%><br>
     <img align="top" width="20" height="20"  src="dis.png"> Distance centre  est :<%=op.GetLocaleCommercial(id_bien).getDistance_center() %><sub> m</sub><br>
     <img align="top" width="20" height="20"  src="locale.png"> Type Locale est :<%=op.GetLocaleCommercial(id_bien).getType_local() %><br>
 <%} %>
 <%if(session.getAttribute("typebien").equals("terrain")){ %>
C'est un bien de type <%=session.getAttribute("typebien")%>,construit :<%=op.GetContenueCol("ajoute4", id_bien)%><br>,
avec une  superficié de <%=op.GetTerrain(id_bien).getSuperficie()%>  <sub>  métre carré</sub> ,et un prix de <%=op.GetTerrain(id_bien).getPrix()%> <sub> DA</sub>
par jour , admis <%=op.GetTerrain(id_bien).getMax_colocataire() %>  <sub>  locataire/s</sub> a la fois ,ce/cette <%=session.getAttribute("typebien") %> est localisé a :<%=op.GetTerrain(id_bien).getAdresse() %> , 
Le <%=op.GetContenueCol("ajoute1", id_bien)%><br>
Si le locataire  de ce bien  retarde le paiement il doit rembourcer:<%=op.GetContenueCol("ajoute2", id_bien)%><br>
Chaque mois le loyer sera payable(le cas du mode paiement est mensuel) :<%=op.GetContenueCol("ajoute3", id_bien)%><br>
   <img align="top" width="20" height="20"  src="climo.png">  Caracteristique est :<%=op.GetTerrain(id_bien).getCaracteristique() %> <br>
 <%} %>
 <%if(session.getAttribute("typebien").equals("locale_commercial")){ %>
  C'est un bien de type <%=session.getAttribute("typebien")%>,construit :<%=op.GetContenueCol("ajoute4", id_bien)%><br>,
avec une  superficié de <%=op.GetLocaleCommercial(id_bien).getSuperficie()%>  <sub>  métre carré</sub> ,et un prix de <%=op.GetLocaleCommercial(id_bien).getPrix()%>   <sub> DA</sub>
...
<form action="GoContratFact" method="get">
